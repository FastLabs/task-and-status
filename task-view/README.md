...
* [`src/cljs/task_view/`](src/cljs/task_view/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/task_view/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `task-view` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/task_view/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns task-view.example
  (:require [task-view.config :as config])
